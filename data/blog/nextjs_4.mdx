...
tags: ['nextjs']
draft: false
summary: 本篇会讲解如何定义一个路由处理程序以及写路由处理程序时常遇到的一些问题。
---
...
request 对象是一个 [NextRequest](xxx) 的扩展。使用 request ，你可以快捷读取 cookies 和处理 URL。
...
![截屏2024-02-29 12.02.47.png](http://blog.minouccc.online:9000/blog/app_router_4.5.png)
...
![截屏2024-02-29 12.37.21.png](http://blog.minouccc.online:9000/blog/app_router_4.6.png)
...
- 使用像 cookies、headers 这样的[动态函数](xxx)
...
注：Next.js 的缓存方案我们还会在 [《缓存篇 | Caching》](xxx)中详细介绍。
...
注：`sitemap.xml`、`robots.txt`、`app icons` 和 `open graph images` 这些特殊的文件，Next.js 都已经提供了内置支持，这些内容我们会在[《Metadata 篇 | 基于文件》](xxx)详细讲到。
