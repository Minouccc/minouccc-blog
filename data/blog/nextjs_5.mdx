...
module.exports = {
  async redirects() {
    return [
      {
        source: '/blog/minou',
        destination: '/blog/minou_redirects',
        permanent: true,
      },
    ]
  },
  async rewrites() {
    return {
      beforeFiles: [
        {
          source: '/blog/minou',
          destination: '/blog/minou_beforeFiles',
        },
      ],
      afterFiles: [
        {
          source: '/blog/minou',
          destination: '/blog/minou_afterFiles',
        },
      ],
      fallback: [
        {
          source: '/blog/minou',
          destination: `/blog/minou_fallback`,
        },
      ],
    }
  },
...
export function middleware(request) {
  return NextResponse.redirect(new URL('/blog/minou_middleware', request.url))
}
...
export default function Page() {
  redirect('/blog/minou_slug')
}
```

现在我们在多个地方都配置了重定向和重写，那么问题来了，现在访问 `/blog/minou`，最终浏览器地址栏里呈现的 URL 是什么？

答案是 `/blog/minou_slug`。按照执行顺序，访问 `/blog/minou`，先根据 `next.config.js` 的 `redirects`重定向到 `/blog/minou_redirects`，于是走到动态路由的逻辑，重定向到 `/blog/minou_slug`。
...
从 URL 的角度来看，`www.yauyjs.com/users/`和 `www.minoujs.com/users`是两个地址，不过通常我们都会做重定向。比如你在 Next.js 中访问 `/about/`它会自动重定向到 `/about`，URL 也会变为 `/about`。
...
![middleware-1.gif](http://blog.minouccc.online:9000/blog/app_router_5.1.gif)
...
![image.png](http://blog.minouccc.online:9000/blog/app_router_5.2.png)
